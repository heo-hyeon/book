...
public class CodeGroupVo {
		...
		private int rowNum = 10;          //화면에 보여줄 데이터 줄 수
		private int pageNum = 5;          //화면에 보여줄 페이징 번호 수
		private int totalRows;
		private int totalPages;
		private int startPage;
		private int endPage;
		private int startRnum = 1;        //쿼리시작 row
		private int endRnum;              //쿼리 끝 row
		private Integer RNUM;
		private int startRnumMysql = 0;   
		...
		public int getStartRnumMysql() {
			return startRnumMysql;
		}

		public void setStartRnumMysql(int startRnumMysql) {
			this.startRnumMysql = startRnumMysql;
		}
		
		//페이징 계산 + 갯수별로 나누는 코드
		public void setParamsPaging(int totalRowsParam) {
			totalRows = totalRowsParam;
			totalPages = totalRows / rowNum;
			...
			startPage = (((thisPage-1) / pageNum) * pageNum + 1);
			...
			endRnum = ((rowNum * thisPage));
			startRnum = ((endRnum - rowNum) + 1);
			if (startRnum < 1) startRnum = 1;
			
			if (thisPage ==1) {
				startRnumMysql = 0;
			} else {
				startRnumMysql = ((rowNum * (thisPage - 1)));
			}
